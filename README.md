...
![Exchange rate page served by nginx, generated by feeder](screenshot.png)

This page is written entirely by `feeder` — it fetches the live rate,
renders it into `data/www/index.html`, and `web` (nginx) just serves
whatever is currently in that file. Or watch the log:
```bash
